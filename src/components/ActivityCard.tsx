	...
	return (
		<Link href={`/activities/${activity.slug}`} className='block'>
			<div className='w-full bg-white border border-gray-200 rounded-lg hover:shadow-lg transition-shadow cursor-pointer overflow-hidden'>
				<div className='aspect-video relative overflow-hidden'>{renderImage()}</div>
				<div className='p-4'>
					<h3 className='font-semibold text-black text-lg leading-tight mb-2'>{activity.name}</h3>
					<p className='text-gray-600 text-sm mb-3'>{activity.description}</p>
					<div className='flex items-center gap-1 mb-3'>
						<Star className='h-4 w-4 fill-yellow-400 text-yellow-400' aria-hidden='true' />
						<span className='text-sm font-medium text-gray-700'>{activity.rating} av 5 stjärnor</span>
					</div>
					...
					{activity.ratingBreakdown && (
						<div className='space-y-2'>
							{[
								...
								const maxValue = Math.max(activity.ratingBreakdown!.fiveStars, activity.ratingBreakdown!.fourStars, activity.ratingBreakdown!.threeStars, activity.ratingBreakdown!.twoStars, activity.ratingBreakdown!.oneStar);
								...
								return (
									<div key={stars} className='flex items-center gap-2'>
										<div className='flex items-center gap-1 min-w-[30px]'>
											...
											<span className='text-xs text-gray-500'>{stars} stjärnor</span>
										</div>
										<div className='flex-1 relative'>
											<div className='w-full h-2 bg-gray-200 rounded-full overflow-hidden'>
												<div className='h-full bg-gray-500 rounded-full transition-all duration-300' style={{ width: `${barWidth}%` }} />
											</div>
										</div>
									</div>
								);
